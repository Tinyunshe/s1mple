...
func (i *Img) Upload(cf string, pageIds []string, token string, client *http.Client, retryCount int, done <-chan bool) {
	<-done
	for _, p := range pageIds {
		...
		url := fmt.Sprintf("%v/rest/api/content/%v/child/attachment", cf, p)
